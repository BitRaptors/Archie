...
Attempted on `/tmp/BabyWeather.Android.pi` (rsync copy of `BabyWeather.Android`) after a fresh Archie install. The first run with the configured OpenAI Codex provider failed after 18.89s with `usage_limit_reached` (429; primary usage 100%). A fallback run with `google/gemini-2.5-flash` failed because the available Google key was invalid for the Generative Language API. No trustworthy Pi-vs-Codex wall-clock ratio was produced in that session. Follow-up work added `.archie/pi_parallel_deep_scan.py`, a transparent RPC-parallel adapter that runs Wave 1 through child `pi --mode rpc --no-session` workers using the declarative job graph at `.archie/prompts/pi/deep_scan_jobs.json`, then writes the standard `.archie/blueprint.json` and runs renderer/validator. The Pi deep-scan body now treats that adapter as an optimization with automatic sequential fallback: it runs the adapter under a full timeout (`ARCHIE_PI_RPC_TIMEOUT`, default 1200s), detects early no-output hangs after 60s, validates that `.archie/blueprint.json` exists and contains all required top-level keys, deletes partial output on any failure, then falls through to the sequential Wave-1 instructions. The final response includes either `deep-scan completed via parallel adapter in Xs` or `deep-scan completed via sequential fallback in Xs (parallel adapter failed: <reason>)`. Adapter orchestration was validated with a fake Pi RPC binary; real wall-clock benchmarking still needs Pi RPC provider quota.
