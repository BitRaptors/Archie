...
def aggregate_verdict(intent_spec: dict, confirmed: list[dict]) -> dict:
    ...
    Completeness uses "silence = met" semantics: edge-A is prompted to flag unmet
    criteria only; a criterion not flagged unmet is counted as met.
    met = total - unmet.  `unknown` is reserved (always 0) pending per-criterion
    verdicts from edge-A.
    ...
    Returns:
        ...
        - drift: count of intent_drift
        - unknown: always 0 (reserved for future per-criterion edge-A verdicts)
