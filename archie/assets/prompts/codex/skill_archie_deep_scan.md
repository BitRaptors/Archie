...
Run Archie’s deep scan end to end from Codex. Prefer named subagents under `.codex/agents/`, but do not fail just because one Codex-specific batching primitive is unavailable.
...
- If `spawn_agents_on_csv` is unavailable, spawn the four Wave 1 agents individually with equivalent instructions.
- If named subagents are unavailable entirely, perform the four Wave 1 perspectives locally and write the equivalent structured results into `/tmp/archie_wave1_results.csv` yourself.
- The deep scan must still complete even when Codex cannot batch subagents automatically.
...
Then use the named agent `archie-wave2-reasoning` for the synthesis pass when available. Give it the scanner outputs plus the full Wave 1 CSV results and ask it to return structured JSON containing:
...
If the named reasoning agent is unavailable, perform the synthesis locally to the same schema.
