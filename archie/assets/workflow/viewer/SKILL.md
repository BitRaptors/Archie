...
The viewer renders the same React UI the share flow uses (`archie-viewer.vercel.app`). It works against whatever data exists — even after just `{{COMMAND_PREFIX}}archie-scan`, you'll see health scores, findings, rules, and the architecture diagram. Sections that need deep-scan data (full component graph, decision chain) render sparse.
