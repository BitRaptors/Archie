...
// Remove all previously-installed shared fragments from .claude/commands/_shared/
// (re-copied fresh below so renames/removals in the npm package propagate cleanly)
const sharedCommandsDir = join(claudeCommands, "_shared");
if (existsSync(sharedCommandsDir)) {
  for (const f of readdirSync(sharedCommandsDir)) {
    if (f.endsWith(".md")) {
      try { unlinkSync(join(sharedCommandsDir, f)); cleanedCount++; } catch {}
    }
  }
}
...
// 2b. Copy shared fragments referenced by the commands above
//     (e.g. _shared/semantic_findings_spec.md — Read at runtime by agent prompts).
//     Glob-style: mirror every .md in assets/_shared/ so new fragments ship without a code change.
const sharedAssets = join(ASSETS, "_shared");
if (existsSync(sharedAssets)) {
  mkdirSync(sharedCommandsDir, { recursive: true });
  for (const f of readdirSync(sharedAssets)) {
    if (!f.endsWith(".md")) continue;
    const src = join(sharedAssets, f);
    const dest = join(sharedCommandsDir, f);
    writeFileSync(dest, readFileSync(src, "utf8"));
    console.log(`  ${GREEN}✓${RESET} ${commandsDirRel}/_shared/${f}`);
  }
}

// 3. Copy standalone Python scripts
for (const script of ["_common.py", "aggregate_findings.py", "scanner.py", "refresh.py", "intent_layer.py", "renderer.py", "install_hooks.py", "merge.py", "finalize.py", "validate.py", "viewer.py", "drift.py", "extract_output.py", "arch_review.py", "measure_health.py", "check_rules.py", "detect_cycles.py", "upload.py", "telemetry.py"]) {
