...
# Archie blueprint nudge — reminds the AI about project architecture on every prompt
PROJECT_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || echo ".")"
...
try:
    ...
    if parts:
        ...
        print('[Archie] Blueprint: .archie/blueprint.json | Rules: .archie/rules.json')
except Exception:
    pass
" 2>/dev/null
exit 0
...
def install(project_root: Path) -> None:
    ...
    _add_hook(user_prompt, "", ".claude/hooks/blueprint-nudge.sh")
