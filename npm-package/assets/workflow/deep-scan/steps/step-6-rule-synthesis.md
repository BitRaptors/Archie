...
> - **Carry the grounding through.** For observed laws, copy `enforced_at` into the rule and set `triggers.path_glob` from it so the hook fires on the right files; put the `failure_mode` in `why`. For derived laws, list the `derived_from` ids in `why` so the agent sees the premises.
>
