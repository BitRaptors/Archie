...
async def generate_image(state: AvatarGeneratorState) -> dict:
    ...
    # --- Image Edit Prompt from DB ---
    _, edit_prompt = resolve_prompt("avatar_image", {
        "character_name": character_name,
        "visual_description": visual_description,
    })
    logging.info(f"[Graph - {character_id}] Using Image Edit prompt: {edit_prompt[:150]}...")
    await send_sse_event(str(character_id), "status", {"step": "prompt_ready", "message": "Image prompt prepared.", "image_prompt": edit_prompt})
