...
class IntentLayerRenderer:
    ...
    # ── Legacy AI-based rendering (kept for optional AI enrichment mode) ──

    def render_folder_claude_md(
        self,
        folder: FolderNode,
        context: FolderContext,
        repo_name: str,
    ) -> str:
        """Render a CLAUDE.md file for a folder from AI-generated FolderContext."""
        lines: list[str] = []
        ...
        if context.cross_references:
            ...
            for ref in context.cross_references:
                ...
                lines.append(f"- [`{path}`]({path}) — {rel}")
            lines.append("")

        # Downlinks (subfolders)
        if context.downlinks:
            lines.append("## Subfolders")
            for dl in context.downlinks:
                path = dl.get("path", "")
    ...
    def render_claude_rule(
        self,
        folder: FolderNode,
        context: FolderContext,
        repo_name: str,
    ) -> str:
        """Render a .claude/rules/*.md file for a folder."""
        glob_path = f"{folder.path}/**" if folder.path else "**"
        lines: list[str] = []

        # YAML frontmatter
        lines.append("---")
        lines.append(f'description: "{folder.path or "root"} conventions"')
        lines.append("paths:")
        lines.append(f'  - "{glob_path}"')
        lines.append("---")
        lines.append("")

        # Title
        lines.append(f"# {folder.name} Rules")
        lines.append("")
    ...
    @staticmethod
    def render_rule_filename(folder_path: str) -> str:
        """Convert folder path to a rule filename.

        "src/api/routes" -> "src-api-routes.md"
        "" (root) -> "root.md"
        """
        if not folder_path:
            return "root.md"
        slug = folder_path.replace("/", "-").replace("\\", "-")
        return f"{slug}.md"
