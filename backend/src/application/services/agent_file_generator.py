...
def generate_claude_md(blueprint: StructuredBlueprint) -> str:
    ...
    lines.append("")
    lines.append("1. Call `list_implementations` to see all known implementation patterns")
    lines.append("2. Match your task to the right capability using IDs and descriptions")
    lines.append("3. Call `how_to_implement_by_id` with the matching ID for full details")
    lines.append("4. Fall back to `how_to_implement` with keyword search if no match")
    lines.append("5. If key files are listed, call `get_file_content` to study the existing implementation")
    lines.append("6. Call `where_to_put` to find the correct location for new files")
    lines.append("7. Call `check_naming` to validate your component names")
    lines.append("")
...
def generate_cursor_rules(blueprint: StructuredBlueprint) -> str:
    ...
    lines.append("")
    lines.append("1. Call `list_implementations` to see all known implementation patterns.")
    lines.append("2. Match your task to the right capability, then call `how_to_implement_by_id` with its ID.")
    lines.append("3. Fall back to `how_to_implement` with keyword search if no exact match.")
    lines.append("4. If key files are listed, call `get_file_content` to study the existing implementation.")
    lines.append("5. Call `where_to_put` before creating or moving any file.")
    lines.append("6. Call `check_naming` before naming any new component (class, function, module, file).")
    lines.append("")
