...
>
>   **Empty by default — fill ONLY when there is a falsifiable invariant a future reader could check by reading the cited line.** Strict tests for whether to leave empty:
>     - Tautological with the capability name → empty. *"Any user-facing string"* / *"Any user action that must be tracked"* / *"A new screen is needed"* / *"A ViewModel needs to toggle gated UI"* are not preconditions — they're paraphrases of the capability's purpose. They state *when the capability applies*, not *what code-level invariant must hold for the recipe to be correct*.
>     - Cannot be expressed as a single, checkable code citation → empty. If the precondition reduces to "follow the recipe" or "you want the thing the capability provides," it isn't an invariant.
>     - Pattern is a workflow recipe, not a contract → empty. "Add a new screen by registering a Module + Fragment + ViewModel" is a recipe — there is no precondition that, if violated, makes the pattern silently wrong.
>     **Fill ONLY when:** misapplying the pattern in a context where the cited invariant does NOT hold would produce silently incorrect code (the openmeter lockr case: cited `(namespace, key)` UNIQUE index ⇒ misapplying to a non-unique key serializes unrelated rows). If you can't articulate that failure mode in one concrete sentence, leave empty.
>
...
> - **applicable_when**: The verifiable invariant that makes this pattern correct in THIS codebase. MUST cite a concrete code artifact at `<file>:<line>` — pick whichever invariant shape fits the language and paradigm: schema annotation (unique/foreign-key/NOT-NULL/index), type signature (Result/Option/exhaustive enum/generic bound), lifecycle state (hook under a Provider, handler registered before bus start), ownership/concurrency (borrow scope, lock-held interval, transaction-active context), or structural contract (single registration point + iterating consumer, sealed hierarchy + exhaustive match). The requirement is that the citation is **falsifiable against the corpus**, not its invariant shape. NOT prose. "Per-customer operations" / "in the auth flow" / "during request handling" are NOT preconditions — they pattern-match across domains where the invariant doesn't hold.
>
>   **Empty by default. Fill ONLY when misapplying the pattern in a context where the cited invariant does NOT hold would produce silently incorrect code.** That's the openmeter lockr test: the (namespace, customerID) unique index makes the lock target one row; applying lockr to App where (namespace, type) is *not* unique silently serializes unrelated rows. The failure mode is articulable in one sentence and verifiable by reading the cited file:line.
>
>   **Reject these as `applicable_when` (leave empty instead):**
>     - Tautologies with the capability name (e.g. "Any user-facing string", "Any user action that must be tracked", "A new screen is needed", "A ViewModel needs to toggle gated UI") — these state *when the capability is wanted*, not *what invariant must hold for the recipe to be correct*.
>     - Workflow recipes (e.g. "Add Module + Fragment + ViewModel registered in startKoin") — no precondition; following the steps gets you the pattern.
>     - Pattern descriptions in disguise (paraphrasing `pattern_description` with extra words).
>
>   On a UI-heavy or recipe-heavy codebase, expect MANY guidelines to have empty `applicable_when` — that is correct. Most patterns are workflows, not invariant-bound contracts. Filling this field on a workflow recipe adds ceremony without signal and degrades the reader's trust in the field as a whole.
