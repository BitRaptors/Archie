...
/** Server-side directory browser card. `onChoose` should throw on failure —
 * the error is rendered inline; on success the parent closes/unmounts this. */
export default function FolderBrowser({ title, subtitle, chooseLabel, onChoose, onCancel }: {
  title: string
  subtitle: string
  chooseLabel: string
  ...
  return (
    <div className="flex h-[34rem] w-[36rem] max-w-[92vw] flex-col rounded-lg border border-border bg-card shadow-sm">
          ...
          <ul className="space-y-0.5">
            {listing.dirs.map((d) => (
              <li key={d.path} className="group flex items-center gap-1">
                ...
                <button onClick={() => choose(d.path)} disabled={busy}
      ...
      <div className="flex items-center justify-between border-t border-border px-5 py-3">
        ...
        <button onClick={() => listing && choose(listing.path)} disabled={!listing || busy}
